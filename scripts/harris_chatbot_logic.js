...
async function getHarrisChatbotResponse(userMessage) {
    console.log('User Message:', userMessage);

    const policies = await loadPoliciesFromFile('harris_policies.json');
    console.log('Loaded Policies:', policies);

    const lowerCaseMessage = userMessage.toLowerCase();
    
    const keywordMap = {
        'immigration': ['immigration', 'migrant', 'border', 'refugee', 'asylum'],
        'economy': ['economy', 'economic', 'job', 'trade', 'tax', 'inflation', 'prices', 'spending', 'wages'],
        ...
        'climate change': ['climate', 'environment', 'green', 'renewable', 'sustainability'],
        'civil rights': ['civil rights', 'equality', 'discrimination', 'voting rights', 'justice'],
        'gun control': ['gun', 'firearm', 'violence', 'safety', 'second amendment'],
        'foreign policy': ['foreign policy', 'international', 'diplomacy', 'military', 'global'],
    };
    ...
    if (relevantPolicies.length > 0) {
        const responses = relevantPolicies.map(policy => {
            ...
            const relevantIdeas = policy.ideas
                ...
                .slice(0, 3);  // Top 3 most relevant ideas
            ...
            return `On ${policy.issue}, V.P. Harris's stance is: ${commitmentSummary}${ideasSummary ? `\n\nKey ideas: ${ideasSummary}` : ''}`;
        });
        return responses.join('\n\n');
    } else {
        return "I don't have specific information on that topic, but I'd be happy to discuss other issues important to the Harris campaign. Try searching with key issues such as immigration, the economy, and education.";
    }
}

module.exports = {
    getHarrisChatbotResponse,
};